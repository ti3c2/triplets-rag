...
Research codebase to test whether teacher-LLM-generated `(query, contexts, answer)` triplets — retrieved as in-context demonstrations — help a smaller student model in RAG, versus retrieving raw chunks at matched context-token budget. `PLAN.md` (~27KB) is the design rationale and the source of truth for *why* things are shaped the way they are; `README.md` mirrors most of the user-facing surface.

## Common commands

Dependency management is via `uv` (Python ≥3.11). The package installs a `triplet-rag` Typer entrypoint.
...
uv run triplet-rag run --config experiment/squad_triplet_pilot \
    -o budget.num_triplets=3 -o retriever.triplet_retrieval_mode=chunk_mediated
...
uv run triplet-rag report --filter experiment_name=squad_*_pilot --metrics em,f1,faithfulness

bash scripts/run_pilot.sh        # vanilla vs triplet vs QuOTE on SQuAD-1k (real OpenAI calls)
